...
data {
  ...
  // PK parameter T_e, half elimination rate, 
  real T_e;
  // Reference regimen (dose and frequency)
  real<lower=0> ref_dose;
  real<lower=0> ref_tau;
  // Number of provisional regimens
  int Nregimens;
  // Provisional regimens
  matrix[Nregimens, 8] regimens;
...
transformed data {
  ...
  // Calculate End-of-cycle-1 log(AUC) to calculate DLT rate of each dose
   for(i in 1:Nregimens) {  
     dose_lamt_tref[i] = rep_vector(regimens[i, 4], 1);
     dose_tau_tref[i] = rep_vector(regimens[i, 8], 1);
   }
...
generated quantities {
  ...
  logke_prior   = normal_rng(0.41, 0.26);
