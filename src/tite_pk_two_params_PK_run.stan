/*
  ...
  TITE-PK Two Parameter model (log beta + k_eff)
...
vector<lower=0>[N] tau;
  int<lower=0, upper=1> dv[N]; // observations, 1=event, 0=censoring for mdv==0 records

  // reference time point (in months) 
  real<lower=0> tref_month;

  // log of pk parameter T_e half elimination rate, 
  real T_e;
...
int x_i[0];
  ...
  //int dose_addl_tref[1,Nregimens];
...
check_addl_dosing_blocked(dose_M, dose_time, dose_tau, dose_addl);
  ...
  // we set the refernce time-point to 3 month, i.e. the average
  // base-line hazard up to tref is used as reference for the hazard
  // of the drugs
  ltref = log(tref_month);
   // To be used for Calculating End-of-cycle-1 log(AUC) to calculate DLT rate of each dose
...
parameters {
  real logbeta;  // cumulative hazard up to tref 
  real logk_e;
}
transformed parameters {
  ...
  vector[Nobs] lh;
  vector[J] lHc;
  // Additonal transformed parameters for Two-parameters model
  vector[2] theta;
  vector[Nobs] lpk;
  vector[Nobs] lpk_auc;
  vector[Ncens] lauc;
...
model {
  ...
  //lH_dose_ref ~ normal(params_prior[1], params_prior[2]);
  ...
  //P_dose_ref ~ beta(0.5, 1);
  
  // prior for k_e parameter
  logk_e  ~ normal(0.41, 0.26);
